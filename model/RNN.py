...
train_iter, vocab = d2l.load_data_time_machine(batch_size, num_steps)  # 载入时间数据
...
def init_rnn_state(batch_size, num_hidden, device):
    return (torch.zeros((batch_size, num_hidden), device=device),)  # 将隐藏状态变成tuple
...
class RNNModelScratch:
    ...
    def __init__(self, vocab_size, num_hidden, device, get_params,
...
num_hidden = 512
net = RNNModelScratch(vocab_size=len(vocab), num_hidden=num_hidden,
                      device=d2l.try_gpu(), get_params=get_params,
                      init_state=init_rnn_state,
                      forward_fn=rnn)


def predict_ch8(prefix, num_pred, net, vocab, device):
    """在prefix后面生成新字符, num_pred表示生成多少个词"""
